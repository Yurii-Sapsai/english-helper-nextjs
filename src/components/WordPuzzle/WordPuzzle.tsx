...
const WordPuzzle = ({ words, setWordPuzzle }: IWordPuzzleProps) => {
    ...
    const getRandomNumber = () => Math.random() - 0.3;
        ...
        <>
            {wordIndex < words.length &&
                ...
                <div className={styles.wrapper}>
                    ...
                    <div className={styles.printWord}>{printWord} <Image src={BackspaceIcon} className={styles.backSpaceIcon} alt='' onClick={() => removeLastLetter(printWord)}/></div>

                    <div className={styles.letterContainer}>
                        {wordInArray
                            ? wordInArray.map((letter: string, index: number) => (
                    ...
                    <div className={styles.checkContainer}>
                        <button onClick={() => setCheckWord(true)} disabled={printWord.length < words[wordIndex].word.length}>Check</button>{checkWord && words[wordIndex].word}
                    </div>
